...
function begin_play(){
	...
	if (player_1.value == "" || player_2.value == ""){
		...
		alert("Make sure name fields are not empty!");
			...
			this.started = true;
			//board_state = [0,-1,-1,-1,0,-1,-1,-1,0]
		}
	else{
	
			alert("Already started. Please Reset Play to start again.");
	}
	console.log(started, board_state, whose_move());
}
...
function play() {
	...
	if (game_started() == true){
		...
		if(table_ids.includes(move_text.value)){

			if(board_state[table_ids.indexOf(move_text.value)] == -1){
	
				if(this.turn == 1) {
					board_state[table_ids.indexOf(move_text.value)] = 1;
					document.getElementById(move_text.value).innerHTML = "X";
					toggle_move();
					...
					board_state[table_ids.indexOf(move_text.value)] = 0;
					document.getElementById(move_text.value).innerHTML = "O";
					toggle_move();
					document.getElementById("turn_info").innerHTML = "Turn for: " + "<b>X</b>";
					turn_count += 1;
					console.log(board_state);
			...
			else if(board_state[table_ids.indexOf(move_text.value)] == "O" || "X"){
				alert("The cell you entered has already been used. Pick again.");
			}
